...
- Diff two adjacent steps to see precisely what that one concept changed:
  `diff -r steps/10-automations steps/11-memory-and-surfaces`.

**You are here: `steps/11-memory-and-surfaces`.** This README describes the
app at this step.
...
| `steps/11-memory-and-surfaces` | Memory and conditional surfaces: one deployed agent whose capabilities vary per caller                             |
| repo root (step 12)            | Embedding & multi-tenancy: the agent in your own app, with your auth and a `scope_id` per tenant                   |
...
See the diff: `diff -r steps/10-automations steps/11-memory-and-surfaces`.
...
| `src/`                                                | The custom React UI (Vite): one screen, `useStoreQuery` + `useToolRun` (seed/reset/Sync/Send) + the chat-trigger Analyze button, the Send-reply and Reset confirm modals, the `useAutomation()` Auto-sync toggle. |
